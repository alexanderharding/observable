...
Deno.test("asAsyncIterable should abort the Observer on iterator return", async () => {
  // Arrange
...
Deno.test("asAsyncIterable return method should abort the Observer and return done", async () => {
  // Arrange
  let subscriptionAborted = false;
  const source = new Observable<number>((observer) => {
    observer.signal.addEventListener("abort", () => {
  ...
  // Start observing by calling next (it will wait since source doesn't emit)
  iterator.next();
...
Deno.test("asAsyncIterable throw method should abort the Observer and reject", async () => {
  // Arrange
  let subscriptionAborted = false;
...
Deno.test("asAsyncIterable should buffer values when emitted faster than consumed", async () => {
  ...
  // Start iteration to activate the Observer
...
Deno.test("asAsyncIterable should only start observing on first next call", async () => {
  // Arrange
