...
WHAT IT DOES:
`never` is an Observable constant that does nothing — never emits values, never returns, never throws. The Observer stays active indefinitely until aborted.
...
// Only way to end the observation:
controller.abort();
```
